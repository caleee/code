...
10. [声明](#十声明)
11. [更新日志](#十一更新日志)
   - [Vagrant相关](#v2026-02-04)
   - [Docker部署](#v2026-02-12) 
...
- 理论上适用于所有基于 Ubuntu 24.04.3 LTS 镜像定制的系统，除官方 Ubuntu 24.04.3 LTS noble-server-cloudimg 镜像外未经测试。
- 部分内容为 AI 生成，多轮测试未发现问题，但不保证准确性和可靠性，仅供研究学习使用，用户请自行承担风险。

## 十一、更新日志

- 2026-01-29：初始版本发布
- 2026-02-04：添加 Vagrant 初始化脚本
- 2026-02-12：添加 Docker 部署脚本

### v2026-02-04
- Vagrant 初始化脚本 for VMware Workstation Pro
...
- **网络环境**：需要网络连接（用于系统更新和配置）

#### 3、脚本用途
- 重置 Vagrant 用户密码并禁用密码认证
- 创建具有 sudo 权限的新用户
- 配置 SSH 密钥认证
- 设置系统时区为亚洲/上海
- 配置阿里云 APT 镜像源（加速国内网络环境下的更新）
- 更新系统软件包
- 配置静态网络
- 自动关机完成初始化
...
#### 4、解决痛点及注意事项：
⚠️ **谨慎使用**
- Vagrant 只能使用NAT网络建立虚拟机第一个网卡，脚本重新配置网卡，模拟云主机 WAN/LAN 单网卡、同网卡场景。
- 脚本执行后会关机，必须在 VMware Workstation Pro 中将虚拟机网卡配置为桥接模式，否则虚拟机网络无法正常使用。
- 脚本会重置 Vagrant 用户密码并禁用密码认证
- 脚本会覆盖现有的网络配置
- 脚本执行完成后会自动关机
- 请确保在执行前备份重要数据
- 请根据实际网络环境调整参数
- 仅在新创建的虚拟机上使用，避免在生产环境中执行

#### 5、参数说明
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `user_name` | 要创建的 sudo 用户名称 | `sudoer` |
| `custom_cidr` | 静态网络 CIDR | `192.168.8.150/24` |
| `custom_gateway` | 网络网关地址 | `192.168.8.1` |
| `custom_nameserver` | DNS 服务器地址 | 与 `custom_gateway` 相同 |

#### 6、使用方法

- 提前将ssh认证公钥信息文件 authorized_keys 放到/ tmp/authorized_keys
- 或使用vagrant创建虚拟机时上传，使用Vagrantfile示例在[这里](config/Vagrantfile)

- **本地执行**

```bash
# 使用默认参数执行
bash 04-vagrant-vmware-workstation-init.sh

# 自定义参数执行
bash 04-vagrant-vmware-workstation-init.sh <用户名> <CIDR> <网关> <DNS服务器>
```

- **一键执行参考**

```bash
curl -sSL https://raw.githubusercontent.com/caleee/ubuntu-cloud-init/main/scripts/04-vagrant-vmware-workstation-init.sh | bash -s -- "sudoer" "192.168.8.150/24" "192.168.8.1"
```

```bash
curl -sSL https://cdn.jsdelivr.net/gh/caleee/ubuntu-cloud-init@main/scripts/04-vagrant-vmware-workstation-init.sh | bash -s -- "sudoer" "192.168.8.150/24" "192.168.8.1"
```

### v2026-02-12
- Docker 部署脚本 for Ubuntu 24.04
...
- 配置 Docker 服务
- 安装 Docker Compose
- 配置 Docker 优化及镜像加速器（示例中的加速器地址不可用，请根据实际情况修改）
...
```bash
curl -sSL https://raw.githubusercontent.com/caleee/ubuntu-cloud-init/main/scripts/05-docker.sh | bash
```

```bash
curl -sSL https://cdn.jsdelivr.net/gh/caleee/ubuntu-cloud-init@main/scripts/05-docker.sh | bash
```
